...
def parse_json_for_bom_ref(sbom_file_path, sbom_project_name):
...
bom_ref_set = parse_json_for_bom_ref(sbom_file_path, sbom_project_name)
...
if not bom_ref_set and not pkg_set or bom_ref_set == pkg_set:
    ...
    # Print unique elements in each set for condition 3
    unique_in_bom_ref = bom_ref_set - pkg_set
    unique_in_pkg_set = pkg_set - bom_ref_set
    print("Unique in BOM-Ref:", unique_in_bom_ref)
    # print("Unique in PKG Set:", unique_in_pkg_set)
